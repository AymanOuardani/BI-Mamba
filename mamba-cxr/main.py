...
def get_args_parser():
    ...
    parser.set_defaults(bce_loss=True)
    ...
    parser.add_argument('--lin4', action='store_true')
    ...
    parser.add_argument('--data-path', default='/datasets01/imagenet_full_size/061417/', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='NLSTDual', choices=['NLSTDual', 'NLST', 'NLSTLateral'],
                        type=str, help='Image Net dataset path')  # todo: experiment on single view dataset
    parser.add_argument('--inat-category', default='name',
                        choices=['kingdom', 'phylum', 'class', 'order', 'supercategory', 'family', 'genus', 'name'],
                        type=str, help='semantic granularity')

    # if use concatenate cls tokens
    parser.add_argument('--cat-cls', action='store_true')  # todo: experiment cls concatenation
...
def main(args):
    ...
    # if use dual-view training mode
    args.dual_mode = True if args.data_set == 'NLSTDual' else False
...
>>>>>>> upstream/main
    ...
    if args.eval:
        test_stats, outall, tarall = evaluate(data_loader_test, model, device, amp_autocast, args.dual_mode, args.cat_cls)
        msg = f"AUROC of the network on the {len(dataset_test)} test images: {test_stats['auroc']:.4f}"
        print(msg)
        with (output_dir / "log.txt").open("a") as f:
            f.write(msg + "\n")
        # np.savez(args.output_dir + '/pred.npz', pred=outall.cpu(), tar=tarall.cpu())
        np.savez(args.output_dir + '/pred_best.npz', pred=outall.cpu(), tar=tarall.cpu())
        return
    ...
    print(f"Start training for {args.epochs} epochs")  # todo
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats, _, _ = evaluate(data_loader_val, model, device, amp_autocast, args.dual_mode, args.cat_cls)
        print(f"AUROC of the network on the {len(dataset_val)} test images: {test_stats['auroc']:.4f}")
        # print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")

        # if max_accuracy < test_stats["acc1"]:
        #     max_accuracy = test_stats["acc1"]
        if max_auroc < test_stats["auroc"]:
            max_auroc = test_stats["auroc"]
            if args.output_dir:
        ...
        print(f'Max AUROC: {max_auroc:.2f}%')
