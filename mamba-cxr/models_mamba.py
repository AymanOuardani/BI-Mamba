"""Vision Mamba (VIM) for chest radiographs"""
import torch
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        assert H % self.img_size[0] == 0 and W % self.img_size[1] == 0, \
            f"Input image size ({H}*{W}) dose not match model ({self.img_size[0]}*{self.img_size[1]})."
...
class VisionMamba(nn.Module):
    def __init__(self,
        ...
        if self.dual_mode and not self.cat_cls:
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False,
        ...
        if not self.if_bidirectional:
            ...
            for layer in self.layers:
                ...
                hs_res_ls.append((hidden_states, residual))
                # todo: return last 4 hidden states: hidden_states are after norm
        else:
        ...
        if not self.fused_add_norm:
            ...
            if self.lin4:
                norm_hs_ls = []
                for hs, res in hs_res_ls[-4:]:
                    hs = fused_add_norm_fn(
                        self.drop_path(hs),
                        self.norm_f.weight,
                        self.norm_f.bias,
                        eps=self.norm_f.eps,
        ...
        if self.if_cls_token:
            if self.use_double_cls_token:
                ...
                if self.use_middle_cls_token:
                    if self.lin4:
                        return torch.cat([h[:, token_position, :] for h in norm_hs_ls], dim=-1)
    ...
    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False,
        ...
        if return_features:
            return x
        if self.cat_cls:
            nb_sample = x.shape[0]//2
            x_cat = []
            for i in range(nb_sample):
                tmp = torch.cat([x[i, :], x[i+nb_sample, :]]).unsqueeze(0)
                x_cat.append(tmp)
            x = torch.cat(x_cat)
...
# @register_model
# def bi_mamba(pretrained=False, **kwargs):
#     model = VisionMamba(
#         patch_size=16, stride=16, embed_dim=384, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
#         final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type='',
#         if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
#     model.default_cfg = _cfg()
#     return model